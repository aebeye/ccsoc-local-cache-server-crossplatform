...
function getBlobs(blobService, blobContainer) {
    ...
    return mkdirp(localContainerDirectory, function (error) {
        ...
        var prefixContentSet = { 'prefix': blobContainer.name === config.k1IncludeDirectories[0] ? config.k1PrefixContentSetFolder
            : identifiedContentSet
        };
...
function syncPackages(packageForGrade) {
    //logger.info ( "syncPackages");

    if (!(packageForGrade === gradePackages.K1 || packageForGrade === gradePackages.K212)) {
        return logger.error("Invalid grade while invoking syncPackages");
    }

    logger.info("SyncPackages invoked with: " + packageForGrade);
    var blobService;
    ...
    if (!nonOnDemandDownloads.length && (config.syncHours.length === 0 || config.syncHours.indexOf((new Date()).getHours()) > -1)) {
        ...
        return blobService.listContainers(function (error, blobContainers, nextMarker) {
            ...
            var includeContainers = u.filter(blobContainers, function(item) {
                ...
                }, { "keys": packageForGrade === gradePackages.K1 ? config.k1IncludeDirectories : config.k212IncludeDirectories }
            );
...
async.series([
    function(callback) {
        configCodeService.getConfigCodeDetails(config.environmentIdentifier, configCodeDir,
            function(err, result) {
                if (err) {
                    return callback(err);
                } else {
                    configCodeFile = result;
                    logger.info("Config code blob details streamed locally to: " + configCodeFile);
                    callback(null);
                }
            });
    },
